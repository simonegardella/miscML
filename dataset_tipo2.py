...
class Dataset ():
    ...
    def populate (self,punti):
        ...
        while len (self.X) < punti:
            ...
            for classeriga in range(self.classi):
                for classecolonna in range(self.classi):
                    classe = (classeriga + classecolonna) % self.classi
                    if classecolonna * separatore <= x  < (classecolonna+1)* separatore and classeriga * separatore <= y  < (classeriga+1)* separatore:
                        self.X.append([x,y])
